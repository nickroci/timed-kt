...
public composing machine of `TimedKt.Comp` through its context argument.
`SucceedsOn` records success of one program on one tape within a time bound; the
clock enters only as an explicit cutoff `∃ t' ≤ t` over the deterministic clocked
relation `CompRuns`, so monotonicity in the bound is definitional (`SucceedsOn.mono`,
...
* `pKt_le_Kt`, `pKt_cond_le_Kt` — **the zero-constant embeddings**: an attained
  ...
  majority of `2 ^ R` out of `2 ^ R`, with the same length and transition count. The
...
proved here. The public machine's erase bits discard the whole context, conditioning
string and randomness together: a plain-probabilistic witness that actually reads
its random tape cannot be transported to the joined context by the erase bit, and
the context-erasing embedding above only reaches the deterministic `Kt`. The statement
...
`CompRuns` — the runs themselves carry exact transition counts, and the
...
theorem HasMajorityAt.one_le_programLength {ctx : BitString → BitString}
  ...
  obtain ⟨v, t', -, hrun⟩ := h.exists_succeedsOn
